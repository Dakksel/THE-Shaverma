/* =====================================================
   THE ШАВЕРМА — menu data + cart + WhatsApp redirect
   ===================================================== */

const PHONE = "77076091750"; // international format, no "+"

/* ---------- 0. CONFIG ---------- */
/* Стоимость доставки и условие бесплатной доставки (отключения)
   пока НЕ заданы — оставлено пустым по запросу.
   Как только определитесь с цифрами, впишите их сюда:
     DELIVERY_THRESHOLD — сумма заказа, начиная с которой доставка бесплатна (₸)
     DELIVERY_FEE        — стоимость доставки, если сумма заказа меньше порога (₸)
   Пока оба значения = null, доставка в корзине показывается как "уточняется"
   и не прибавляется к итоговой сумме. */
const DELIVERY_THRESHOLD = null;
    ...
    ${g.items.map((item)=>{
      ...
      return `
        <div class="card ${off?'out-of-stock':''}">
          <div class="card-body">
            <h3>${item.name}</h3>
            ${off ? `<p class="out-badge">Нет в наличии</p>` : (item.desc ? `<p class="desc">${item.desc}</p>` : `<p class="desc">&nbsp;</p>`)}
...
function updateCart(){
  ...
  /* delivery fee — TODO: заполнить DELIVERY_THRESHOLD / DELIVERY_FEE выше, когда определитесь с суммой */
  let fee = 0, hint = "", hintClass = "";
  if(total === 0){
    fee = 0; hint = "";
  } else if(DELIVERY_FEE === null || DELIVERY_THRESHOLD === null){
    fee = 0;
    hint = "стоимость доставки уточнит оператор";
  ...
  $("#deliveryFee").textContent = (DELIVERY_FEE === null) ? "уточняется" : (fee ? `+${fee.toLocaleString("ru-RU")} ₸` : "бесплатно");
